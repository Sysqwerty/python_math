...
c = Canvas(root, width=450, height=300, bg='white') # Розмір і колір полотна
c.pack()
...
# Визначення координат розміщення вершин на полотні
tops = [
...
def edge_kn(n1,n2,k,n):
    ...
    # Якщо максимальний і мінімальний час виконання не збігаються (є резерв) — малюємо зелену стрілочку, якщо збігається (ребро відноситься до критичного маршруту)
...
# Визначаємо максимальний та мінімальний час закінчення робіт
q = []
n_max = [0,0,0,0,0]
n_min = [0,0,0,0,0]
q.append(0)
